...
func main() {
	...
	api := r.Group("/api/v1")
	{
		...
		{
			articles.GET("", article.List)          // 获取多个文章
			articles.GET("/:id", article.Get)       // 获取单个文章
			articles.POST("", article.Create)       // 创建文章
			articles.PUT("/:id", article.Update)    // 更新文章
			articles.DELETE("/:id", article.Delete) // 删除文章
		}
	}
